...
import type { Position, DivId } from './interfaces.tsx';
import './App.css';
...
function App() {
    ...
    const [positionsArrive, setPositionsArrive] = useState<{ [key in DivId]: Position }>({
        div1: { x: 0, y: 0 },
        div2: { x: 0, y: 0 },
        div3: { x: 0, y: 0 }
    });
    ...
    function animate(targetId: DivId, destination: Position, duration: number = 1000) {
        ...
        function smoothAnimate(currentTime: number) {
            ...
            if (progress < 1) {
                animationFrameIds.current[targetId] = requestAnimationFrame(smoothAnimate);
            } else {
    ...
    const handleChange = (divId: DivId, axis: 'x' | 'y') => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleChangeArrive = (divId: DivId, axis: 'x' | 'y') => (e: React.ChangeEvent<HTMLInputElement>) => {
        let numVal = e.target.value === "" ? 0 : Number(e.target.value);
        if (numVal >= screenWidth && axis === "x") numVal = 0;
        if (numVal >= screenHeight && axis === "y") numVal = 0;
        ...
        <>
            <div className="flex flex-col justify-around">
                {/* <div
                    ...
                    style={{ transform: `translate(${positions.div1.x}px, ${positions.div1.y}px)` }}
                ...
                <div
                    ...
                    style={{ transform: `translate(${positions.div2.x}px, ${positions.div2.y - 100}px)` }}
            ...
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 flex">
                <div className="flex flex-nowrap flex-col">
                    <div className="flex flex-row flex-nowrap my-auto">
                        ...
                        <input type="number" className="border-1 border-solid mx-3 border-slate-600 rounded-2xl text-black px-5" id="inputDiv2X" placeholder="Div2 départ X" onChange={handleChange("div2", "x")}/>
                        <input type="number" className="border-1 border-solid mx-3 border-slate-600 rounded-2xl text-black px-5" id="inputDiv2Y" placeholder="Div2 départ Y" onChange={handleChange("div2", "y")}/>
                    </div>
                    <div className="flex flex-row flex-nowrap my-5">
                        <input type="number" className="h-10 border-1 border-solid mx-3 border-slate-600 rounded-2xl text-black px-5" id="inputDiv1XArr" placeholder="Div1 arrivée X" onChange={handleChangeArrive("div1", "x")}/>
                        <input type="number" className="border-1 border-solid mx-3 border-slate-600 rounded-2xl text-black px-5" id="inputDiv1YArr" placeholder="Div1 arrivée Y" onChange={handleChangeArrive("div1", "y")}/>
                        <input type="number" className="border-1 border-solid mx-3 border-slate-600 rounded-2xl text-black px-5" id="inputDiv2XArr" placeholder="Div2 arrivée X" onChange={handleChangeArrive("div2", "x")}/>
                        <input type="number" className="border-1 border-solid mx-3 border-slate-600 rounded-2xl text-black px-5" id="inputDiv2YArr" placeholder="Div2 arrivée Y" onChange={handleChangeArrive("div2", "y")}/>
                    </div>
                </div>

                <div className="flex flex-col flex-nowra mx-10">
                    <button className="button_mov button mx-3 p-2 bg-emerald-500 text-white rounded" onClick={() => animate('div1', positionsArrive.div1)}>
                        Bouger Div 1
                    </button>
                    <button className="button_mov mx-3 p-2 bg-gray-500 text-white rounded" onClick={() => animate('div2', positionsArrive.div2)}>
                        Bouger Div 2
                    </button>
                    <button className="button_mov mx-3 p-2 bg-blue-500 text-white rounded" onClick={() => {
